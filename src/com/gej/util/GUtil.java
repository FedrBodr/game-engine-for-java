...
/**
 * Some of the utilities needed by the engine. Note that all the utility methods are static. This
 * class is necessary for the engine to run. The class is made abstract to prevent the user from
 * instantiation.
...
public abstract class GUtil {
    ...
    /**
     * You specify the number of sides of a dice which is then thrown. Then if the dice lands on
     * one, the result is true and the next action is performed. This can be used to put an element
     * of randomness in your game. For example, in each step you can generate with a particular
     * chance a bomb or a change of direction. The larger the number of sides of the dice, the
     * smaller the chance.
