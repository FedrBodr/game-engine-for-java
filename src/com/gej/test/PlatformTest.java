...
public class PlatformTest extends Game implements MapLoader {
	...
	public void initResources() {
		// load the background
		Background.setBackground(ImageTool.resize(
		        loadImage("resources/back_water.png"), Global.WIDTH,
		        Global.HEIGHT));
		// load the font
		fpsfont = GFontAdvanced.getFont(loadImage("ImageFonts/font_blue.png"),
		        "ImageFonts/DefFontDescriptor.txt");
		...
		public void update(long elapsedTime) {
			...
			if (jump_started && jump_time <= 1000) {
				...
				jump_time += elapsedTime;
			        ...
			        .isObjectCollisionFree(getX(), getY() + 1, true, this)) {
			...
			if ((GKeyBoard.isPressed(KeyEvent.VK_SPACE) || GKeyBoard
			        .isPressed(KeyEvent.VK_UP)) && !jump_started) {
	...
	public class Enemy extends GObject {
		...
		public void update(long elapsedTime) {
			...
			if (MapView.isVisible(this)) {
				boolean canMoveRight = Map.isObjectCollisionFree(getX() + 1,
				        getY(), true, this);
				boolean canMoveDown = Map.isObjectCollisionFree(getX(), getY(),
				        true, this);
				if (canMoveRight)
					setVelocityX(0.15f);
				if (canMoveDown)
	...
	public class Coin extends GObject {
		...
		public Coin(int x, int y) {
			super(new Animation(ImageTool.splitImage(
			        loadImage("resources/coin.png"), 1, 32), 100));
			setX(x);
			setY(y);
		}
