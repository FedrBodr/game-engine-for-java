...
/**
 * Loads and plays wav files. To decrease the delay on loading of the files,
 * you are recommended to load them using the static loadSound() method. You
 * can play them again at any time by the invoking the play() method
 * of the loaded sound. Supports playing simultaneous files. Supports *.wav *.au *.aiff
 * files. Also you can use MIDI files which are detected and converted automatically.
	        ...
	        while (running){
	        	...
	        	if ((sound.getState() == SoundState.STATE_PLAYING) || (sound.getState()==SoundState.STATE_LOOPING)){
	        		...
	        		if (numBytesRead!=-1){
	        			...
	        			// The samples are ended. So reset the position of the stream
	        			is.reset();
