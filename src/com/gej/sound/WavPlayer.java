...
/**
 * Loads and plays wav files. To decrease the delay on loading of the files, you are recommended to
 * load them using the static loadSound() method. You can play them again at any time by the
 * invoking the play() method of the loaded sound. Supports playing simultaneous files. Supports
 * *.wav *.au *.aiff files. Also you can use MIDI files which are detected and converted
 * automatically. But if you want to modify the MIDI data, use the MidiPlayer class instead.
 * @author Sri Harsha Chilakapati
 */
public class WavPlayer extends Thread {
    
    /**
     * Loads a sound from a file in the jar file to play them at any time. Supports loading of .wav,
     * .au, .aiff files. Also converts MIDI files on the fly
