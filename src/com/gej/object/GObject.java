...
public class GObject implements Updateable {
	...
	public Rectangle getBounds() {
		return new Rectangle(Math.round(getX()), Math.round(getY()),
		        getWidth(), getHeight());
	}
	...
	public boolean isCollidingWith(GObject other) {
		...
		if (bool && Global.USE_PIXELPERFECT_COLLISION) {
			bool = GUtil.isPixelPerfectCollision(x, y, getAnimation()
			        .getBufferedImage(), other.getX(), other.getY(), other
			        .getAnimation().getBufferedImage());
		}
		return bool;
	}
	 ...
	 */
	public void moveToContact(GObject other, boolean horizontal,
	        boolean vertical) {
		if (horizontal) {
