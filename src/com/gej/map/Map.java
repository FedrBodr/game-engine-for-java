...
public abstract class Map {
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				for (int j = 0; j < lines[i].length(); j++) {
					...
					objects.add(loader.getObject(mapdata[j][i], j * TILE_SIZE,
					        i * TILE_SIZE));
					_tiles[j][i] = loader.getTile(mapdata[j][i], j * TILE_SIZE,
					        i * TILE_SIZE);
				}
			}
		} catch (Exception e) {
	 ...
	 */
	public static GObject getCollidingObject(float x, float y, int width,
	        int height) {
		GObject obj = null;
		Rectangle bounds = new Rectangle(Math.round(x), Math.round(y), width,
		        height);
	 ...
	 */
	public static boolean isObjectCollisionFree(float x, float y,
	        boolean solid, GObject object) {
		boolean bool = true;
		Rectangle bounds = new Rectangle(Math.round(x), Math.round(y),
				...
				try {
					if (bounds.intersects(obj.getBounds())) {
						...
						if (bool && Global.USE_PIXELPERFECT_COLLISION) {
							bool = GUtil.isPixelPerfectCollision(x, y, object
							        .getAnimation().getBufferedImage(), obj
							        .getX(), obj.getY(), obj.getAnimation()
							        .getBufferedImage());
							if (bool) {
								bool = false;
							}
	...
	public static boolean isObjectCollisionFree(float x, float y, GObject object) {
		return (isObjectCollisionFree(x, y, true, object) || isObjectCollisionFree(
		        x, y, false, object));
	}
				...
				for (int mx = 0; mx < MAP_WIDTH; mx++) {
					...
					if (tile != null) {
						if (MapView.isVisible(tile)) {
							g.drawImage(tile.getImage(), tile.getX() + x,
							        tile.getY() + y, null);
						}
					}
				}
	 ...
	 */
	private static final void checkCollisions(GObject obj, boolean hor,
	        boolean vert) {
