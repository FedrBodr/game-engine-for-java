 ...
 * write a game. The games requires J2SE 1.5 or more to run. This class won't
 * support applet mode as applets are not the target of this engine. This class
 * uses triple buffering if available or uses double buffering. The default game
...
public abstract class Game extends JPanel implements Runnable, Updateable {
		...
		GUtil.runInSeperateThread(new Runnable() {
			public void run() {
				while (running) {
					...
					try {
						Thread.sleep(Math.min(1000 / Global.FRAMES_PER_SECOND,
						        10));
					} catch (Exception e) {
					}
					repaint();
	...
	public final void paint(Graphics g) {
		try {
			...
			if (backImage == null) {
				backImage = ImageTool.getColoredImage(getBackground(),
			...
			if (Global.FULLSCREEN) {
				g2D.drawImage(backImage, 0, 0, GWindow.RESOLUTION_X,
				        GWindow.RESOLUTION_Y, null);
			} else {
				g2D.drawImage(backImage, 0, 0, Global.WIDTH, Global.HEIGHT,
				        null);
			}
			g2D.dispose();
	...
	public static Image loadImage(String name) {
		...
		if (cache.containsKey(name)) {
			...
			img = new ImageIcon(Game.class.getClassLoader().getResource(name))
			        .getImage();
			cache.put(name, img);
		}
		return img;
